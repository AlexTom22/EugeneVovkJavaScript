/**
 ...
 - Лучше когда пользователь может сам задавать те параметры,
 которые будут входить в проверку
 - Часто прилетает цифра строкой и прежде чем делать какуие-то преобразования,
 нам надо эту цифру проверить и преобразовать
 - Не стоит забывать, что по стандарту ноль это ложное значение,
 следовательно для преверки используем функцию isNaN
 - А так же что пустая строка преобразовывается в ноль,
 следовательно, в первую очередь делаем проверку на пустое значение
