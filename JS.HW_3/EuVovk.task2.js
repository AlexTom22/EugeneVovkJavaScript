...
/**
 * Метод возарвщает массив уникальных объектов
 * @param cards Принимает массив
 * @param uniqueField Принимает строку, которая берётся за уникальность
 */
function uniquePersonCards(cards, uniqueStr) {
     const set = new Set();
     return cards.filter(el => {
         if (set.has(el[uniqueStr])) {
             return false;
         }
         set.add(el[uniqueStr]);
         return true;
     });
 }

 //console.log(uniquePersonCards(array, 'email'));
 //console.log(array.length);
 //console.log(uniquePersonCards(array, 'email').length);
