 ...
 * Возвращает список пользователей в консоль
 * @param array Принимает массив объектов
 * @param data Принимает данные для поиска
 */
function findUserByDate(array, ...data) {
    for (let i = 0; i < array.length; i++) {
        for (let j = 0; j < data.length; j++) {
            if (array[i].registrationDate == data[j]) {
                console.log(array[i]);
            }
        }
    }
...
//findUserByDate(jsonUser, '09.10.2021', '10.10.2021');
